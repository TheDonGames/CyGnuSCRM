                ...
                <tr>
                  <td colSpan={7} className="py-16">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm font-medium text-gray-700 dark:text-slate-300">No profiles found</p>
                    </div>
                  </td>
                </tr>
              ) : (
      ...
      {actionMenuId && (
        <>
          ...
          <div
            ...
            } rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-[#131b2e] py-1.5 shadow-xl animate-fade-in`}
    ...
    <>
      <td className="px-4 py-2.5">
        <div className="flex items-center gap-2.5">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-400 text-xs font-semibold">
            {row.name.charAt(0).toUpperCase()}
          </div>
          <span className="text-sm font-medium text-gray-900 dark:text-slate-100 truncate">{row.name}</span>
        </div>
      </td>
      <td className="px-4 py-2.5 text-xs text-gray-600 dark:text-slate-400 font-mono">{row.phone}</td>
      <td className="px-4 py-2.5 text-xs text-gray-500 dark:text-slate-400">{row.email}</td>
      <td className="px-4 py-2.5 text-sm font-medium text-gray-900 dark:text-slate-100 text-right">{row.total_tickets}</td>
      <td className="px-4 py-2.5 text-sm font-semibold text-gray-900 dark:text-slate-100 text-right tabular-nums">{row.total_spent.toLocaleString()} LBP</td>
...
function EditProfileDrawer({ modal, customer, staff, onSave, onClose }: any) {
  const [form, setForm] = useState<Record<string, string>>(() => {
    if (modal.type === 'customers' && customer) return { name: String(customer.name ?? ''), phone: String(customer.phone ?? ''), email: String(customer.email ?? '') };
    if (modal.type === 'staff' && staff) return { username: String(staff.username ?? ''), password: String(staff.password ?? ''), email: '' };
    return {};
  });
      ...
      <aside className="relative z-10 flex w-full max-w-md flex-col bg-white dark:bg-[#131b2e] shadow-2xl animate-slide-in-right border-l border-gray-200 dark:border-slate-800">
        ...
        <div className="flex items-start justify-between px-6 py-5 border-b border-gray-200 dark:border-slate-800">
          <div>
            ...
            <h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100 leading-tight">Edit — {subtitle}</h2>
